...
> ④ **三道保守化护栏（防"不可撤回的假 true"，由生产 4 条误点亮驱动）**：**G1 口径不对应就不判**（常量表 `_NON_PRICE_DOMAIN_KEYWORDS`：海外宏观〔美债/美元指数/美股/纳指/标普/恒生/人民币汇率/美联储/加息/降息/原油〕、情绪家数〔涨停/跌停/炸板/封板/连板/家数/赚钱效应〕、资金流〔净流入/净流出/主力资金/北向/融资余额/融券〕；命中且 anchor 无**对应** metric → 整体 `None` 不产键；刻意不误伤 成交额/成交量/换手率/支撑位/前低/MA20）；**G2 复合条件不得半判**（按 `且/并且/同时/而且/以及` 切子句——**不按中文逗号**，逗号多用于并列同一子句内对象；≥2 子句须**全部可判且全部成立**才点亮，任一判不了 → `None`）；**G3 方向动词 + 百分数不确定就不判**（`is_dir_verb_pct_ambiguous`：下跌/上行动词〔跌破/下破/失守/站上/突破/收回，与 `_TECH_RE` 逐字同集〕+ 百分数、且**未明示技术位**〔均线/日线/周线/月线/MA\d+/前低/新高〕→ 整体 `None`——`metric` 缺省即 `close` 不携带口径信息，会被文本兜底错归技术位类并用 MA20 近似判出假 true；**刻意不含 上穿/击穿**，二者本就不进技术位判径）。事件类（`anchor.event_ref`）在判定体**首行短路**，三护栏均不介入。
> ⑤ **口径**：`source_id` = `sector:{resolved 权威名}:{YYYY-MM-DD}`（级联与批量两路统一；落库前 `list_predictions(source_id)` fail-safe 跳过，防重复）；`condition_met_rate` = True 数 / **含布尔 `condition_met` 的 entry 数**，且**仅当出现 ≥1 个 `false` 时计算**（否则 `None`）。
