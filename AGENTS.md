...
> ⑦ **手动验证入口**：`POST /api/agent/admin/trigger/review_full`（**需 `X-Internal-Token`**）+ JSON body `{"report_date":"YYYY-MM-DD","with_chain":true}` → 复盘 `status=ok` 后自动发布 `review_done`（缺省总线，不可用则临时 RedisPool 建总线），链与级联预判一并触发，返回体含 `chain_published`；`with_chain` 缺省 `false` 时既有行为逐字不变。
> ⑧ **未闭环/遗留**：生成侧 `condition` 文本百分数与 `anchor.threshold` **不一致**（R22，id=24 实证文本 -3% vs 锚 -4%）→ 收敛后可评估把 G3 放宽为涨跌幅口径；`trigger_review_quick` 未联动（结构同缺）；**弱反馈应用层未实现**（`attribution_feedback_mode` 仅 `off`/`observe`，只落建议不改写溯源，真正加权待立项）。
