...
>
> **重大事件时间线 Event Entity（2026-09-15，agent-py 侧接入，spec 冻结版 §4.2/§5A.3/§5B.3/§10.2）**：P0 做 app-api 权威 `event_id` 透传——`EventRecord` 增可选 `app_event_id`（既有 `event_id` 降级为 **source_event_id** 语义）；`event_scraper._trigger_conduction` 条件透传（缺省不落键，major_events 形状逐字节不变）；`event_conduction.run_single_event_conduction` 优先消费 `app_event_id` 作传导隔离键（缺省回退 `evt_md5`=execution_id，逐字节不变）；`event_persister` content 加性回写 `appEventId`。P0.5 做确定性事件时间抽取（`event_scrape_sources._extract_event_start_time`，只认明确绝对日期/区间，区间取开始日，禁 LLM 猜日期）+ 未来事件物化（`_materialize_event_entity` → `node_api.post_event_entity`，挂接在 `scrape_full_daily`/`scrape_intraday` 的 added_events 后）。开关 `event_entity_enabled`（env `EVENT_ENTITY_ENABLED`，默认 **False**）：app-api `/internal/event-entities` 端点落地前全链路短路、不发起 HTTP。降级纪律：物化失败 warning、不阻断抓取/传导主链路。`event_status`/canonical_key 语义 Merge 由 app-api 负责（agent-py 不实现不传）；已发生事件物化 + `time_confidence` 阈值判定列入联调前置对账清单。跨仓：`aistock-app-api`/`aistock-app-frontend` 生产代码 0 改动。
