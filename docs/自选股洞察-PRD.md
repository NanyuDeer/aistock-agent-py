...
> **已确认：二期不包含"盘中触及跌停"事件。** 同花顺原创无稳定跌停雷达文章来源，跌停池数据源 `limit_list_ths` 需 8000 积分暂不纳入（见 §16）；下跌方向的异动由午盘/尾盘价格异动 `direction = down`（§5.2）覆盖，涨跌停家数等市场冲击指标不再依赖跌停池（见 §9）。
...
### 建议方案：冻结证据包（已确认：约束归因 + 证据时效分层）

午盘、尾盘 7% 异动没有已经确认的逐股实时解读等价来源。触发时由 Node.js 冻结证据包，Python 只能在该证据范围内归并。归因保持一期"候选集约束的 LLM 选择 + 规则校验兜底"架构（不采用 LLM 联网自由归因：来源不可追溯、结果不稳定、违反可审计要求）。

#### 证据时效分层（已确认：约束归因 + 时效分层，参数经实证统计校准）

| 时效层 | 时间范围 | 纳入类型 | 时效系数 |
|---|---|---|---|
| T0 | T-1 交易日 至 触发时刻（含盘前/隔夜） | 公司公告 / 业绩预告 / 新闻 / 研报 | T-1: 1.0 / 当日: 0.8 |
| T1 | T-2 ~ T-5 交易日 | 重要公告 / 研报 / 政策 | 0.6 → 0.3 递减 |
| T2 | T-6 ~ T-10 交易日 | 仅重要类型（重大合同 / 重组 / 政策） | 0.2 |
| 业绩特例 | 业绩公告 / 业绩预告可追溯至 T-20 | earnings 类 | 0.3 → 0.1 |

- **T0 覆盖"T-1 交易日至触发时刻"完整时间线**（含隔夜与盘前），无断层；午盘（11:30）T0 = T-1 至 11:30，尾盘（15:05）T0 = T-1 至 15:05（含上午新增，增量更新，与 §5.3"补抓新证据更新"呼应）
- **实证依据**（2026-08 抓取 mrnxgg_list 204 篇涨停雷达）：原因日期相对涨停日分布——当天 7.9% / T-1 14.0%（最高频单日）/ T-2~T-5 15.4% / T-6~T-10 5.7% / 10 天以上 57.4%（含大量背景引用：互动易、旧研报、历史数据，需 LLM 候选集约束区分"背景支撑"与"直接主因"）；业绩公告实证可影响至 T-20（如 7/15 业绩预告超预期 → 8/7 涨停，diff=23 天）
- 候选证据携带 `published_at`，`strength` 乘时效系数（T-1 权重最高，与"前一天信息是最常见原因时点"实证一致）
- **置信度联动**：主因证据来自 T1/T2（或业绩特例远期）时置信度上限为中/低；T0 无直接证据且 T1 无重要事件 → unconfirmed
- 远期引用作为候选但不自动降级为"主因"，由 LLM 候选集内选择时权衡（近期直接证据优先）
...
数据源：Tushare 同花顺板块（`ths_member` 成分股 / `ths_daily` 板块日线）、Tushare `daily`（个股日线）、腾讯 6 大指数与市场广度（`TencentSnapshotService` / `MarketSnapshotService`）。**二期不再依赖 Tushare `limit_list_ths`（涨跌停池计数）**——随"盘中触及跌停"事件一并剔除（见 §16）。

| 判定 | 阈值 | 说明 |
|---|---|---|
| 板块强度 | 目标股所属同花顺行业板块当日 `pct_change >= 3%`（`ths_daily`） | 板块显著走强/走弱 |
| 同行同步 | 同板块成分股中当日同向且涨跌幅 `>= 5%` 的股票数量 `>= 3` 只 | 多只同行同步异常 |
| 市场冲击（任一满足且与个股方向同向） | ① 上证指数当日 `abs(pct_change) >= 1.5%`；② 上涨家数占比 `<= 30%` 或 `>= 70%` | ①与现有晨报市场事件推送阈值（±1.5%）对齐；原③涨跌停家数（`>= 50` / `>= 20`）依赖 `limit_list_ths`，随跌停异动剔除，二期不再作为市场冲击判定项 |
...
**L2 量化联动证据权重（已调整）**：板块强度与同行同步依赖 Tushare 同花顺板块数据（`ths_member` / `ths_daily`，均需 8000 积分）。**积分权限已实测确认（2026-08-07）**：`ths_index`/`ths_member`/`ths_daily`/`limit_list_ths` 均正常返回（TrendScore/IndustryKG/RotationBoard 等生产功能亦在调用），L2 证据按中置信度可用。若后续积分降级导致 L2 不可用，权重回落——`industry_theme` 置信度改由 L1 正文直接证据（高/中）或 L3 标题标签（低）决定；`market` 仅由市场冲击 ①② 支撑（腾讯来源，不依赖积分）。涨跌停家数指标已随跌停异动剔除，不再参与 `market` 判定。
...
**已确认：不包含"盘中触及跌停"事件**（无稳定跌停雷达文章来源，跌停池数据源 `limit_list_ths` 需 8000 积分暂不纳入，见 §16）；下跌方向异动由午盘/尾盘 `direction = down` 覆盖。
...
- 是否存在稳定的跌停雷达或等价来源 — 已确认：未找到稳定"跌停雷达"文章来源（同花顺原创及 `mrnxgg_list` 栏目当前均仅涨停雷达文章）。**二期明确剔除"盘中触及跌停"事件**：Tushare `limit_list_ths` 跌停池（需 8000 积分）不再作为二期依赖，留待积分具备后评估三期候选（复用午盘/尾盘归因机制，无解读文章时输出待验证）；
...
- 行业联动、同行数量和市场冲击阈值 — 已确认：板块强度 ≥3%、同行同步 ≥3 只且同向 ≥5%、市场冲击（上证 ±1.5% / 广度 ≤30% 或 ≥70%），原涨跌停家数指标随跌停异动剔除，详见第 9 节；
