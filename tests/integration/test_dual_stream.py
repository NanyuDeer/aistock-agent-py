...
async def test_queue_fanout_single_graph_execution():
    ...
    messages 流触发 graph 执行（call_count=1），updates 流从同一 graph 的事件
    序列中过滤工具事件。由于 Queue 在 messages 流结束后被清理，updates 流
    通过预填充相同事件序列来验证过滤逻辑。
    """
    ...
    # ── updates 流：预填充相同事件序列，验证过滤逻辑 ──
    # （在生产中，updates 与 messages 共享同一 queue 并发读取；
    #   测试中 messages 已消费完毕且 queue 已清理，故预填充验证 updates 的过滤行为）
