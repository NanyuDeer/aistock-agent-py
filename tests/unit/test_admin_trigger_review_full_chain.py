"""POST /admin/trigger/review_full 与 review_quick 的 with_chain 联动：复盘成功后补发 review_done。
...
quick 入口（`ENDPOINT_QUICK`）为同一组用例的逐条镜像（结构对称，日志键/返回体字段同名）。
"""
...
ENDPOINT_QUICK = "/api/agent/admin/trigger/review_quick"
...
def _review_result(
    ...
    return type(
        ...
        (),
        {
            ...
            "trace_id": trace_id,
            "markdown": f"# {snapshot_kind.title()}",
        },
    )()
...
def _post_quick(client: TestClient, payload: dict | None = None):
    return client.post(
        ENDPOINT_QUICK,
        headers=AUTH_HEADERS,
        json=payload if payload is not None else {"report_date": "2026-09-17"},
...
def _quick_review_result(status: str = "ok"):
    """quick 版 run_review 结果（snapshot_kind=quick，trace_id 用 manual-quick-* 前缀）。"""
    return _review_result(
        status=status, trace_id="manual-quick-123", snapshot_kind="quick"
    )
...
# ── /admin/trigger/review_quick 的对称联动（逐条镜像上方 review_full 用例） ──


def test_review_quick_with_chain_absent_keeps_existing_behavior(client) -> None:
    """with_chain 缺省（body 无该键）→ 不触碰总线、不发布（既有行为不变）。"""
    with (
        patch(
            "aistock_agent.agents.workers.review.run_review",
            new=AsyncMock(return_value=_quick_review_result()),
        ),
        patch(
            "aistock_agent.services.event_bus.get_default_bus", new=MagicMock()
        ) as get_bus,
...
def test_review_quick_with_chain_true_publishes_on_default_bus(client) -> None:
    """with_chain=true 且 status=ok → 用会话内默认总线发布一次（幂等 id 由 _publish 负责）。"""
    bus = MagicMock()
    with (
        patch(
        ...
        resp = _post_quick(client, {"report_date": "2026-09-17", "with_chain": True})
    ...
    assert publish.await_args.kwargs == {
        "report_date": "2026-09-17",
        "trace_id": "manual-quick-123",
    }


def test_review_quick_with_chain_true_does_not_publish_when_status_not_ok(client) -> None:
    """status≠ok（降级/跳过）→ 不发布（对齐调度路径"仅 status=ok 发 review_done"硬约束）。"""
    with (
        patch(
            "aistock_agent.agents.workers.review.run_review",
            new=AsyncMock(return_value=_quick_review_result(status="degraded")),
        ),
        patch(
            "aistock_agent.services.event_bus.get_default_bus",
            new=MagicMock(return_value=MagicMock()),
...
def test_review_quick_with_chain_falls_back_to_temp_bus_using_pool_client(client) -> None:
    """无默认总线（lifespan 未建消费者）→ 复用 RedisPool 单例客户端临时建总线发布；单例不关。"""
    redis = _fake_redis()
    with (
        patch(
...
def test_review_quick_with_chain_temp_connection_from_settings_url_when_pool_missing(
    client,
) -> None:
    """RedisPool 未初始化（RuntimeError）→ 按 settings.redis_url 临时连，发布后关闭。"""
    redis = _fake_redis()
...
def test_review_quick_with_chain_publish_failure_does_not_affect_response(client) -> None:
    """发布链路抛异常 → 只 warning，已完成的复盘结果照常返回（chain_published=False）。"""
    with (
        patch(
            "aistock_agent.agents.workers.review.run_review",
    ...
    body = resp.json()
    assert body["status"] == "ok"
    assert body["trace_id"] == "manual-quick-123"
    assert body["chain_published"] is False


def test_review_quick_with_chain_invalid_date_still_422(client) -> None:
    """请求体升级为带 with_chain 的模型后，report_date 校验行为不变（非法 → 422）。"""
    with patch(
        "aistock_agent.agents.workers.review.run_review", new=AsyncMock()
    ) as run:
        resp = _post_quick(client, {"report_date": "invalid", "with_chain": True})
