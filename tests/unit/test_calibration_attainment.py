...
def test_compute_event_attainment_rates() -> None:
    ...
    cases = [
        {"best_score": 0.85, "gt_confidence": "high", "rounds": 5},
        {"best_score": 0.85, "gt_confidence": "low", "rounds": 3},   # low GT 不算达标
        {"best_score": 0.6, "gt_confidence": "medium", "rounds": 8},
        {"best_score": 0.9, "gt_confidence": "medium", "rounds": 10},
    ]
    ...
    assert stats["max_rounds_exhausted"] == 1        # rounds == max_rounds 的 case 数
...
def test_collect_event_cases_skips_top_level_array_files(tmp_path: Path) -> None:
    ...
    # 轮文件：c3 实际跑了 3 轮（_r1_baseline + _r2 + _r3）
    (exps / f"{c3}_r1_baseline.json").write_text('{"score": 0.5}', encoding="utf-8")
    (exps / f"{c3}_r2.json").write_text('{"score": 0.7}', encoding="utf-8")
    (exps / f"{c3}_r3.json").write_text('{"score": 0.8}', encoding="utf-8")
    ...
    assert collected[0]["rounds"] == 3  # 实际轮数 = 轮文件数（非 best_round=1）
