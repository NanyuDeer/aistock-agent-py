...
def _clean_l3_daily_count() -> None:
    """用例间就地清空软上限计数：换引用（`= {...}`）会击穿随机排序下的状态隔离。"""
    src._l3_daily_count.clear()
    yield
    src._l3_daily_count.clear()
...
@pytest.mark.asyncio
async def test_invalid_calendar_dates_dropped(cache: SearchCache, monkeypatch: pytest.MonkeyPatch) -> None:
    """日历合法性：13月45日/2月30日等非法日期 → hit 不入 events，无 post_calendar_event 调用。"""
    def fake_search(query, **kwargs):
        if "财经日历" in query:
            return {"results": [
                {"title": "下周财经日历", "content": "2026年13月45日 发布", "url": "https://x.example/1"},
                {"title": "下月数据", "content": "2月30日 9:30 发布", "url": "https://x.example/2"},
            ], "provider": "anysearch", "outcome": "ok"}
        return {"results": [], "provider": "anysearch", "outcome": "empty"}
