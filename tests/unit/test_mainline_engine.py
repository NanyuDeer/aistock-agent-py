...
def test_load_ok(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    ...
    (tmp_path / "mainline_candidates.json").write_text(json.dumps({
        "candidates": [
            {"name": "AI 算力", "group": "ai_tech", "priority": 1,
             "tag_code": "885896.TI", "aliases": []},
            {"name": "低空经济", "group": "default", "priority": 2,
             "tag_code": "885938.TI", "aliases": []},
        ]
    }), encoding="utf-8")
...
def test_load_missing_returns_false_and_does_not_cache(
